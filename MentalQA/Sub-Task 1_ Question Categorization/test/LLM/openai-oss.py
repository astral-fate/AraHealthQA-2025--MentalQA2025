import os
import pandas as pd
from groq import Groq # MODIFIED: Import Groq
...
# --- Model Configuration ---
# MODIFIED: Updated model name for Groq
...
def build_prompt(question):
    ...
    # This prompt structure is kept the same, as it provides clear instructions to the model.
    prompt = f"""
...
**Your Task:**
Classify the following question. **It is crucial to assign all relevant categories, as a question can have multiple correct labels.** Provide your reasoning and then the final answer as a comma-separated list.
...
def get_prediction_from_groq_api(client, question, all_labels, model_name):
    """Gets a multi-label prediction from the Groq API."""
    full_prompt = build_prompt(question)
    full_response_content = ""

    try:
        # MODIFIED: Updated API call to match the user's request for Groq
        completion = client.chat.completions.create(
            ...
            temperature=1.0,
            max_tokens=8192, # Note: a very high max_tokens might not be necessary
            top_p=1.0,
            stream=True,
            stop=None
        )
        ...
        # The parsing logic remains the same, as we expect a similar output format.
...
def main():
    ...
    for _, row in tqdm(test_df.iterrows(), total=test_df.shape[0], desc=f"Classifying with {MODEL_NAME.split('/')[0]}"):
        # MODIFIED: Call the new Groq API function
        prediction = get_prediction_from_groq_api(client, row['Question'], all_labels, MODEL_NAME)
    ...
    evaluate_predictions(
        ...
        test_df['Predicted_Labels'].tolist(),
        all_labels
    )
